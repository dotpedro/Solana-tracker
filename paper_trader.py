# --- START OF FILE paper_trader.py ---

#!/usr/bin/env python3
"""
Paper Trading Simulator for Solana Tokens based on Monitor Alerts.
Version: 1.9.5 - Added Monitored Holder Filter & Increased Verbosity.
"""
...
if not (TELEGRAM_TOKEN_TRADER and TELEGRAM_CHAT_ID_TRADER): print("[!] WARNING: Trader Telegram credentials missing.")
if not BIRDEYE_API_KEY: print("[!] CRITICAL: BIRDEYE_API_KEY missing."); exit()

try: import api_usage_limiter; print("[✓] paper_trader: Imported api_usage_limiter.")
except ImportError: print("CRITICAL: api_usage_limiter.py not found!"); exit()
try: from helius_analyzer import format_timestamp as default_format_timestamp; print("[✓] paper_trader: Imported format_timestamp.")
except ImportError:
    print("[!] paper_trader: Could not import format_timestamp. Using basic internal version.")
...
TRADER_STATE_FILE   = os.path.join(REPORTS_ROOT, "paper_trader_state.json")
TRADE_LOG_CSV_FILE = os.path.join(REPORTS_ROOT, "paper_trade_log.csv")
...
STRICT_REQUIRE_POSITIVE_1H_CHANGE = True
STRICT_MIN_MONITORED_HOLDERS = 2 # <<< New: Min number of *our* wallets holding
# --- Scoring Weights ---
SCORE_WEIGHTS = { 
    "liquidity": 1.5, 
    "vol_liq_ratio": 1.0, 
    "price_change_1h": 1.5, 
    "total_holders": 0.5, 
    "monitored_holders": 1.2, # <<< Increased weight
    "top1_holder_pct": -1.0, 
    "age_score": 0.5, 
    "socials": 0.3 
}
...
def send_trader_telegram(text: str) -> bool:
    ...
    if not token or not chat_id: print(f"--- TRADER CONSOLE ---\n{text}\n--------------------"); return True
    url = f"https://api.telegram.org/bot{token}/sendMessage"
    # --- REMOVED parse_mode ---
    payload = {"chat_id": chat_id, "text": f"📈 Paper Trader 📉\n\n{text}", "disable_web_page_preview": "true"} 
    # --- END REMOVAL ---
    try: r = requests.post(url, data=payload, timeout=20); r.raise_for_status(); return True
    except requests.exceptions.HTTPError as e:
        error_content = e.response.text[:500] if e.response else str(e); print(f"    [✗] Trader Telegram HTTP error: {e.response.status_code if e.response else 'N/A'} - {error_content}"); print(f"        Problematic text (first 100 chars): {text[:100]}"); return False
    except Exception as e: print(f"    [✗] Trader Telegram general error: {e}"); print(f"        Problematic text (first 100 chars): {text[:100]}"); return False

# (load/save functions, calculate_percent_change, get_latest_report_paths remain the same)
...
def get_latest_report_paths(n: int = 10) -> List[str]: 
    ...
    except Exception as e: print(f"    [!] Error finding latest report paths: {e}"); return []

# (Keep original _format_compact and _format_pct)
...
# (API Call Functions: get_current_price, pt_get_..., get_enriched_data... remain the same)
def get_current_price(mint_address: str) -> Optional[Decimal]:
    ...
    data = api_usage_limiter.request_api(method=requests.get, api_name="birdeye", endpoint_name="price", calling_script="paper_trader.get_current_price", cache_key=cache_key, cache_ttl_seconds=price_cache_ttl, url=url, headers=headers, timeout=10)
    if isinstance(data, dict) and data.get("success") == True and isinstance(data.get("data"), dict):
        ...
        if price_value is not None:
            ...
            except Exception as e: print(f"    [!] Error converting price {price_value} to Decimal for {mint_address}: {e}"); return None
    elif isinstance(data, dict) and data.get("success") == False:
         msg = data.get('message', 'Unknown error')
         if "Not found" not in msg and "Request failed" not in msg: print(f"    [!] Birdeye price API failed for {mint_address}: {msg}")
    return None
def pt_get_bds_market_data(mint: str, chain: str = "solana") -> Optional[Dict]:
    ...
    market_data_response = api_usage_limiter.request_api(method=requests.get, api_name="birdeye", endpoint_name="v3_market_data", calling_script="paper_trader.pt_get_bds_market_data", cache_key=cache_key, cache_ttl_seconds=45, url=url, headers={"X-API-KEY": BIRDEYE_API_KEY})
...
def pt_get_bds_trade_data(mint: str, chain: str = "solana") -> Optional[Dict]:
    ...
    trade_data_response = api_usage_limiter.request_api(method=requests.get, api_name="birdeye", endpoint_name="v3_trade_data", calling_script="paper_trader.pt_get_bds_trade_data", cache_key=cache_key, cache_ttl_seconds=60, url=url, headers={"X-API-KEY": BIRDEYE_API_KEY})
...
def pt_get_bds_metadata_single(mint: str, chain: str = "solana") -> Optional[Dict]:
    ...
    metadata_response = api_usage_limiter.request_api(method=requests.get, api_name="birdeye", endpoint_name="v3_metadata_multiple", calling_script="paper_trader.pt_get_bds_metadata_single", cache_key=cache_key, cache_ttl_seconds=60*60, url=url, headers=headers)
...
def pt_get_bds_token_security(mint: str, chain: str = "solana") -> Optional[Dict]:
    ...
    security_response = api_usage_limiter.request_api(method=requests.get, api_name="birdeye", endpoint_name="token_security", calling_script="paper_trader.pt_get_bds_token_security", cache_key=cache_key, cache_ttl_seconds=5*60*60, url=url, headers=headers)
...
def get_enriched_data_for_pending_candidate(mint_address: str) -> Optional[Dict]:
    print(f"        Fetching enriched data snapshot for {mint_address[:6]}...")
    ...
    if not enriched_data["bds_market_data"] or not enriched_data["bds_trade_data"] or not enriched_data["bds_security_data"]: print(f"        [!] Failed to fetch essential enriched data for {mint_address[:6]}. Market={bool(enriched_data['bds_market_data'])}, Trade={bool(enriched_data['bds_trade_data'])}, Security={bool(enriched_data['bds_security_data'])}"); return None
    ...
    print(f"        [✓] Enriched data snapshot fetched for {mint_address[:6]}."); return enriched_data

# (log_trade_to_csv, _calculate_metrics_from_trades, calculate_performance_metrics remain the same)
...
def log_trade_to_csv(trade_data: Dict[str, Any]):
    ...
    try:
        with open(TRADE_LOG_CSV_FILE, 'a', newline='', encoding='utf-8') as csvfile:
            ...
            writer.writerow(row)
    except IOError as e: print(f"    [✗] Error writing to trade log CSV: {e}")
    except Exception as e_csv: print(f"    [✗] Unexpected error logging trade to CSV: {e_csv}")
def _calculate_metrics_from_trades(trades_list: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for trade in trades_list:
        ...
        except (TypeError, ValueError) as ve: print(f"    [!] Invalid data for metrics calculation in trade {trade.get('mint')}: {ve}. PNL USD: '{trade.get('pnl_usd')}', Entry USD: '{trade.get('position_size_usd')}'")
        except Exception as e: print(f"    [!] Error processing trade for metrics calculation: {trade.get('mint')} - {e}")
...
def passes_strict_filters(item: Dict[str, Any], current_time: int) -> Tuple[bool, Optional[str]]:
    ...
    # print(f"    [Filter Check] Running strict filters for {symbol} ({mint_addr[:6]}...)") # Verbose Entry Log
    ...
    # --- NEW FILTER: Minimum Monitored Holders ---
    # This relies on original_data being present (i.e., called on data from a processed report)
    if original_data and "total_holders_in_set" in original_data: 
        ...
        if not isinstance(monitored_holders_count, int) or monitored_holders_count < STRICT_MIN_MONITORED_HOLDERS:
            print(f"    [Filter Fail] {symbol} ({mint_addr[:6]}): Failed Monitored Holders ({monitored_holders_count} < {STRICT_MIN_MONITORED_HOLDERS})")
            return False, f"Low Monitored Holders ({monitored_holders_count} < {STRICT_MIN_MONITORED_HOLDERS})"
    # else: # Log if check is skipped (e.g., during re-validation)
        # print(f"    [Filter Skip] {symbol} ({mint_addr[:6]}): Skipping monitored holder check (original_data unavailable).")
    ...
    if age_delta_seconds is None: 
        print(f"    [Filter Fail] {symbol} ({mint_addr[:6]}): Failed Age (Cannot Determine)")
        return False, "Cannot Determine Age"
    if age_delta_seconds < (STRICT_MIN_AGE_MINUTES * 60): 
        print(f"    [Filter Fail] {symbol} ({mint_addr[:6]}): Failed Age ({age_delta_seconds/60:.1f} min < {STRICT_MIN_AGE_MINUTES})")
        return False, f"Too New (< {STRICT_MIN_AGE_MINUTES} min)"
        
    total_holders_bds = trade_data.get("holder")
    if not isinstance(total_holders_bds, int) or total_holders_bds < STRICT_MIN_TOTAL_HOLDERS: 
        print(f"    [Filter Fail] {symbol} ({mint_addr[:6]}): Failed Total Holders ({total_holders_bds} < {STRICT_MIN_TOTAL_HOLDERS})")
        return False, f"Low Total Holders ({total_holders_bds} < {STRICT_MIN_TOTAL_HOLDERS})"
    ...
    if not isinstance(liquidity, (int, float)) or liquidity < STRICT_MIN_LIQUIDITY_USD: 
        ...
        print(f"    [Filter Fail] {symbol} ({mint_addr[:6]}): Failed Liquidity ({liq_fmt} < {min_liq_fmt})")
    ...
    if top_holder_pct_val is not None and (not isinstance(top_holder_pct_val, float) or top_holder_pct_val >= STRICT_MAX_TOP1_HOLDER_PCT):
        ...
        print(f"    [Filter Fail] {symbol} ({mint_addr[:6]}): Failed Top Holder % ({pct_disp} >= {STRICT_MAX_TOP1_HOLDER_PCT*100:.0f}%)")
    ...
    if REQUIRE_SOCIALS:
        ...
        if not has_social: 
            print(f"    [Filter Fail] {symbol} ({mint_addr[:6]}): Failed Socials Check")
            return False, "Missing Social Links"
            
    if bds_security.get('mutableMetadata') == True: 
        print(f"    [Filter Fail] {symbol} ({mint_addr[:6]}): Failed Security (Mutable Metadata)")
        return False, "Mutable Metadata"
    if bds_security.get('freezeable') == True and bds_security.get('freezeAuthority') and bds_security['freezeAuthority'] not in ["11111111111111111111111111111111", "TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA"]: 
        print(f"    [Filter Fail] {symbol} ({mint_addr[:6]}): Failed Security (Freeze Authority Active)")
        return False, "Freeze Authority Active"
    if gmgn_security and gmgn_security.get('is_honeypot') == True: 
        print(f"    [Filter Fail] {symbol} ({mint_addr[:6]}): Failed Security (GMGN Honeypot)")
        return False, "GMGN Honeypot"
    if gmgn_security and gmgn_security.get('is_mintable') == True: 
        print(f"    [Filter Fail] {symbol} ({mint_addr[:6]}): Failed Security (GMGN Mintable)")
        return False, "GMGN Mintable"
        
    if STRICT_REQUIRE_POSITIVE_1H_CHANGE:
        price_change_1h_pct = trade_data.get("price_change_1h_percent")
        if not isinstance(price_change_1h_pct, (int, float)) or price_change_1h_pct < 0: 
            pct_fmt = _format_pct(price_change_1h_pct)
            print(f"    [Filter Fail] {symbol} ({mint_addr[:6]}): Failed Price Change ({pct_fmt})")
            return False, f"Negative 1h Price Change ({pct_fmt})" 
            
    # If we got here, all checks passed
    # print(f"    [Filter Pass] {symbol} ({mint_addr[:6]}): Passed all strict filters.") # Can be verbose
    return True, None

def rank_trade_candidates(candidate_items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    # (Increased monitored_holder weight already applied in config)
    scored_candidates = []
    if not candidate_items: return []
    print(f"    [*] Ranking {len(candidate_items)} candidates...") # Log entry
    for item in candidate_items:
        ...
        price_1h = trade_data.get("price_change_1h_percent")
        if isinstance(price_1h, (int, float)): momentum_score = max(Decimal(0), min(Decimal(str(price_1h)) / Decimal("20.0"), Decimal("1.5"))); score += Decimal(str(SCORE_WEIGHTS["price_change_1h"])) * momentum_score;
        else: momentum_score = Decimal(0) 
        if momentum_score > 0: reasons.append(f"1h{float(price_1h):+.1f}%") 
        ...
        # Use the increased weight from config
        if monitored_holders > 5: score += Decimal(str(SCORE_WEIGHTS["monitored_holders"])) * Decimal("1.0"); reasons.append(f"Mon>{monitored_holders}") # Show actual count
        elif monitored_holders > 2: score += Decimal(str(SCORE_WEIGHTS["monitored_holders"])) * Decimal("0.5"); reasons.append(f"Mon>{monitored_holders}")
    ...
    # Log top N candidates after ranking
    print(f"    [*] Ranking Complete. Top {min(3, len(scored_candidates))} candidates:")
    for i, item in enumerate(scored_candidates[:3]):
        print(f"        {i+1}. {item.get('symbol','???')} ({item.get('token_mint')[:6]}...) Score: {item['_score']:.2f} (Reasons: {', '.join(item.get('_score_reasons',[]))})")
    return scored_candidates

def manage_portfolio(state: Dict[str, Any]):
    # Add initial log
    if 'open_positions' in state and state['open_positions']:
        print(f"    [*] Managing {len(state['open_positions'])} open positions...")
    else:
        # print("    [*] No open positions to manage.") # Optional: less noise
        return 

    mints_to_check = list(state['open_positions'].keys()); closed_positions_this_cycle = 0
    ...
    for mint in mints_to_check:
        ...
        # print(f"        Checking open position: {symbol} ({mint[:6]})") # Verbose log
        current_price = get_current_price(mint); time.sleep(0.1) 
        if current_price is None: print(f"    [!] Could not get current price for open position {symbol}. Skipping checks."); continue
        ...
        current_price_fmt_log = _format_compact(current_price, '$', 6); effective_sl_fmt_log = _format_compact(effective_stop_loss_price, '$', 6); trailing_sl_fmt_log = _format_compact(trailing_stop_price, '$', 6); tp_price_fmt_log = _format_compact(take_profit_price, '$', 6)

        if current_price <= effective_stop_loss_price:
            exit_reason = "STOPPED_OUT"; exit_price = current_price * (Decimal("1") - SIMULATED_SLIPPAGE_PCT); print(f"    [!] STOP LOSS triggered for {symbol} at price {current_price_fmt_log} (Effective SL: {effective_sl_fmt_log}, Trail: {trailing_sl_fmt_log})"); state['last_stop_loss_timestamp'] = current_time
        elif current_price >= take_profit_price:
            exit_reason = "TAKE_PROFIT"; exit_price = current_price * (Decimal("1") - SIMULATED_SLIPPAGE_PCT); print(f"    [+] TAKE PROFIT triggered for {symbol} at price {current_price_fmt_log} (TP: {tp_price_fmt_log})")
        # else: # Optional log for no action
            # print(f"        Position {symbol}: Price {current_price_fmt_log} within SL/TP.")

        if exit_reason and exit_price is not None: 
            closed_positions_this_cycle += 1
            exit_value_usd = position_size_tokens * exit_price; exit_value_usd_after_fee = exit_value_usd * (Decimal("1") - SIMULATED_FEE_PCT); pnl_usd = exit_value_usd_after_fee - position_size_usd_entry; pnl_pct = (pnl_usd / position_size_usd_entry) * 100 if position_size_usd_entry > 0 else Decimal("0")
            ...
            if exit_reason == "STOPPED_OUT" and abs(pnl_pct / 100) >= KILL_SWITCH_LOSS_PCT:
                 kill_switch_until = current_time + KILL_SWITCH_DURATION_SECONDS; state['kill_switch_active_until_ts'] = kill_switch_until
                 kill_switch_msg = f"🚨 *KILL SWITCH TRIGGERED!* 🚨\nTrade: {symbol_safe}\nLoss: {pnl_usd_fmt} USD ({pnl_pct_fmt}%)\nHalting new trades for {KILL_SWITCH_DURATION_SECONDS // 60} mins." 
                 print(f"    [!!!] KILL SWITCH TRIGGERED by {symbol} loss ({pnl_pct:.1f}%). Halting new trades until {default_format_timestamp(kill_switch_until)}."); send_trader_telegram(kill_switch_msg)
            ...
            state['trade_history'].append(closed_trade_data); log_trade_to_csv(closed_trade_data)
            ...
            alert_text = (f"Closed {symbol_safe} ({mint_short_safe})\n"
                          ...
                          f"Entry: {entry_price_fmt} | Exit: {exit_price_fmt}\n" 
    ...
    if closed_positions_this_cycle > 0: 
        print(f"    [*] Closed {closed_positions_this_cycle} position(s) this cycle.")
        calculate_performance_metrics(state) # Update overall metrics
...
def check_and_update_risk_level(state: Dict[str, Any]):
    ...
    if current_risk_level == 0 and current_balance >= initial_capital * RISK_LEVEL_1_ENTRY_THRESHOLD:
        new_risk_level = 1; msg = f"🚀 Risk Level Increased: 0 -> 1\nBalance: {balance_fmt} USD"; print(f"    [!] Risk Level Increased: 0 -> 1 (Balance {current_balance:.2f} >= {initial_capital * RISK_LEVEL_1_ENTRY_THRESHOLD:.2f})"); send_trader_telegram(msg)
    if current_risk_level == 1 and current_balance >= initial_capital * RISK_LEVEL_2_ENTRY_THRESHOLD: 
        new_risk_level = 2; new_quota_status = True; msg = f"🏆 Daily Quota Achieved! 🏆\nRisk Level: 1 -> 2\nBalance: {balance_fmt} USD\nNo new trades today."; print(f"    [!] DAILY QUOTA ACHIEVED! Risk Level: 1 -> 2 (Balance {current_balance:.2f} >= {initial_capital * RISK_LEVEL_2_ENTRY_THRESHOLD:.2f})"); send_trader_telegram(msg)
...
def enter_paper_trade(mint: str, symbol: str, name: str, entry_price: Decimal, state: Dict[str, Any]) -> bool:
    ...
    if simulated_entry_price <= 0: print(f"    [!] Invalid simulated entry price ({_format_compact(simulated_entry_price, '$', 8)}) for {symbol}."); return False
    ...
    print(f"    [+] Entered PAPER TRADE for {symbol} ({mint[:6]}...)"); print(f"        Size: {position_size_usd:.2f} USD ({position_size_tokens:.4f} {symbol})"); print(f"        Entry: ~{log_entry_price_fmt} (Target: {log_target_price_fmt})"); print(f"        Initial SL: {log_sl_fmt} (-{STOP_LOSS_PCT:.0%})"); print(f"        TP: {log_tp_fmt} (+{tp_pct:.0%})"); print(f"        Risk Level: {risk_level}")
...
def manage_pending_entries(state: Dict[str, Any]):
    ...
    # Add initial log
    print(f"    [*] Managing {len(state['pending_entry'])} pending entries...")
    ...
    for mint in mints_to_process:
        ...
        # print(f"        Checking pending: {symbol} ({mint[:6]})") # Verbose log
        
        if current_time > monitoring_start_time + entry_window_seconds: print(f"    [-] Pending entry window expired for {symbol} ({mint[:6]}). Removing."); del state['pending_entry'][mint]; continue
        if (state['daily_quota_achieved'] or state.get('daily_loss_limit_hit', False) or len(state['open_positions']) >= MAX_OPEN_POSITIONS or current_time < state.get('last_stop_loss_timestamp', 0) + STOP_LOSS_COOLDOWN_SECONDS or current_time < state.get('kill_switch_active_until_ts', 0)): 
            # print(f"        Skipping checks for {symbol} due to operational halt.") # Verbose log
            continue
            
        current_price_quick = get_current_price(mint); time.sleep(0.05) 
        if current_price_quick is None: 
            print(f"    [!] Could not get quick price for pending entry {symbol}. Retrying next cycle.")
            continue
            
        price_condition_met = False
        if signal_price_str and signal_price_str != 'N/A': 
            try: 
                signal_price = Decimal(signal_price_str); lower_bound = signal_price * (Decimal(1) - PENDING_ENTRY_PRICE_DEVIATION_PCT); upper_bound = signal_price * (Decimal(1) + PENDING_ENTRY_PRICE_DEVIATION_PCT); price_condition_met = (lower_bound <= current_price_quick <= upper_bound)
                if not price_condition_met:
                     print(f"        Price {_format_compact(current_price_quick, '$', 6)} for {symbol} outside deviation of {_format_compact(signal_price, '$', 6)}.")
            except Exception as e: print(f"    [!] Error checking price deviation for {symbol}: {e}"); continue 
        else: 
            print(f"    [?] Signal price missing/invalid for {symbol}, considering price condition met for re-validation.")
            price_condition_met = True 
            
        if not price_condition_met: continue
        
        # Only fetch full data if price condition met
        print(f"    [>] Price condition met for {symbol}. Fetching full data for re-validation...")
        fresh_enriched_data = get_enriched_data_for_pending_candidate(mint)
        if not fresh_enriched_data: print(f"    [!] Failed to fetch fresh enriched data for {symbol}. Skipping entry attempt this cycle."); continue
        
        passes_final_check, reason = passes_strict_filters(fresh_enriched_data, int(current_time))
        if not passes_final_check: print(f"    [!] {symbol} ({mint[:6]}) failed final strict filter re-validation before entry: {reason}. Removing from pending."); del state['pending_entry'][mint]; continue
        
        print(f"    [✓] {symbol} ({mint[:6]}) passed final re-validation.")
        current_price_from_market = (fresh_enriched_data.get("bds_market_data") or {}).get("price")
        if current_price_from_market is not None:
            try: price_to_enter = Decimal(str(current_price_from_market)); print(f"        Using entry price from fresh market data: {_format_compact(price_to_enter, '$', 6)}")
            except Exception as e: print(f"        [!] Error converting market data price {current_price_from_market} to Decimal. Using quick price. Error: {e}"); price_to_enter = current_price_quick
        else: print(f"        [!] Price missing in fresh market data for {symbol}. Using quick price."); price_to_enter = current_price_quick
        
        trade_name = fresh_enriched_data.get("name", pending_data.get('name', 'N/A'))
        print(f"    [*] Attempting to execute paper trade for {symbol}...") # Log before execution attempt
        if enter_paper_trade(mint, symbol, trade_name, price_to_enter, state): print(f"    [>>>] Successfully executed paper trade entry for {symbol} after re-validation."); del state['pending_entry'][mint] 
        else: print(f"    [!] Failed to execute paper trade entry for {symbol} despite passing re-validation.")

# --- Summary Function (Original Markdown) ---
def generate_and_send_performance_summary_md_original(state: Dict[str, Any], period_hours: int, period_name: str, current_time: int):
    # (Function remains the same as 1.9.4)
    ...
    if not period_trades: print(f"    [i] No trades in the last {period_hours} hours to summarize."); summary_msg += f"No trades closed in this period.\n\n" 
    else:
...
def main_trading_loop():
    ...
    print(f"--- Starting Paper Trader (v1.9.5 - Monitored Holder Filter & Verbose Logging) ({start_time_str}) ---") # Version updated
    ...
    print(f"Strict Filter: Min Monitored Holders = {STRICT_MIN_MONITORED_HOLDERS}") # Log new filter value
    print(f"Score Weight: Monitored Holders = {SCORE_WEIGHTS['monitored_holders']}") # Log updated weight

    state = load_trader_state()
    if not state: # Initialize state
        print("[*] Initializing new trader state.")
    ...
    print(f"[*] Current State: Balance=${Decimal(state['current_balance_usd']):.2f}, Risk Level={state['risk_level']}, Quota Met={state['daily_quota_achieved']}")
    print(f"[*] Tracking {len(state.get('open_positions', {}))} open positions, {len(state.get('pending_entry', {}))} pending entries.")
    ...
    while True:
        ...
        print(f"\n--- Loop Start: {default_format_timestamp(current_time)} ---") # Log loop start
        try:
            # --- Daily Reset Check ---
            ...
            if state.get('last_quota_reset_timestamp', 0) < today_start_ts:
                print(f"\n--- New Day [{today_start_utc.strftime('%Y-%m-%d')}] Resetting Daily Quota & Risk Level ---")
                ...
                save_trader_state(state) # Save state after reset
            ...
            print("    [*] Checking operational halts...")
            ...
            if state['open_positions']:
                 ...
                 for mint_pos, pos_data in state['open_positions'].items(): 
                     ...
                     if current_price_for_pnl:
                         ...
                         except Exception as dec_err: print(f"        [!] Error calculating unrealized PNL for {mint_pos}: {dec_err}")
                     time.sleep(0.05)
            ...
            if daily_drawdown >= DAILY_LOSS_LIMIT_PCT:
                if not state.get('daily_loss_limit_hit', False):
                    ...
                    print(f"    [!!!] DAILY LOSS LIMIT HIT! Drawdown: {daily_drawdown:.2%} >= {DAILY_LOSS_LIMIT_PCT:.1%}. Halting new entries."); send_trader_telegram(f"🛑 Daily Loss Limit Hit ({daily_drawdown_fmt})! 🛑\nNo new trades today.\nEffective Balance: {eff_balance_fmt} USD"); state['daily_loss_limit_hit'] = True
            ...
            # Log halt status
            print(f"        Daily Quota: {state['daily_quota_achieved']}, Loss Limit: {state.get('daily_loss_limit_hit', False)}, Trading Hours: {is_trading_hours}, SL Cooldown: {is_sl_cooldown}, Kill Switch: {is_kill_switch_active}")
            print(f"        --> Can consider new trade? {can_consider_new_trade}")
            ...
            print(f"    [*] Should scan reports? {should_scan_reports} (can_consider: {can_consider_new_trade}, time_check: {current_time > last_report_scan_time + CHECK_INTERVAL_SECONDS * 2})")
            
            if should_scan_reports: 
                print(f"\n[*] Scanning last {NUM_REPORTS_TO_SCAN} reports for new candidates...")
                ...
                print(f"    Found {len(latest_report_files)} report files to scan.")
                if not latest_report_files: print("    [!] No processed reports found to scan.")
                else:
                    processed_item_count = 0
                    passed_filter_count = 0
                    for report_path in latest_report_files:
                        report_name = os.path.basename(report_path)
                        # print(f"    Scanning report: {report_name}...") # Can be too verbose
                        report_data = load_json_data(report_path);
                        if not report_data: print(f"        [!] Failed to load report {report_name}"); continue
                        items_list = [];
                        ...
                        if not items_list: print(f"        No relevant items found in {report_name}"); continue
                        
                        # print(f"        Found {len(items_list)} items in {report_name}. Filtering...")
                        processed_item_count += len(items_list)
                        for item in items_list:
                            mint = item.get("token_mint");
                            if not mint or not isinstance(item, dict): continue 
                            passed_strict, fail_reason = passes_strict_filters(item, current_time) # Filter log moved inside function
                            if passed_strict: 
                                if mint not in potential_candidates: # Only count unique passes
                                    passed_filter_count += 1
                                potential_candidates[mint] = item # Keep latest version
                        
                    print(f"    Scanned {len(latest_report_files)} reports, processed {processed_item_count} items total.")
                    print(f"    Found {len(potential_candidates)} unique candidates passing strict filters.")

                    if potential_candidates:
                        items_to_rank = list(potential_candidates.values())
                        ranked_candidates = rank_trade_candidates(items_to_rank) # Ranking log moved inside function
                        ...
                        print(f"    [*] Processing top {len(ranked_candidates)} ranked candidates for pending list...")
                        ...
                        for i, ranked_item in enumerate(ranked_candidates):
                            ...
                            if pending_mint in state['open_positions']: print(f"            Skipping: Already in open positions."); continue 
                            if pending_mint in state.get('recently_pending', {}): print(f"            Skipping: In recently_pending cooldown."); continue 
                                
                            num_open_plus_pending = len(state['open_positions']) + len(state.get('pending_entry', {})) # Recalculate current state
                            ...
                            if pending_mint in pending_entry_map:
                                ...
                                if new_candidate_score > existing_pending_score * PENDING_REPLACE_SCORE_IMPROVEMENT_FACTOR:
                                    print(f"            Updating PENDING {symbol_for_log} ({pending_mint[:6]}) with better score ({new_candidate_score:.2f} > {existing_pending_score:.2f} * {PENDING_REPLACE_SCORE_IMPROVEMENT_FACTOR:.2f})")
                                ...
                                else: print(f"            Skipping: Already pending with similar/worse score.")
                            elif has_free_slot:
                                print(f"            -> Added {symbol_for_log} ({pending_mint[:6]}) to PENDING (slot available). Score: {new_candidate_score:.2f}")
                                ...
                                if lowest_score_pending_mint and new_candidate_score > lowest_score: 
                                    ...
                                    print(f"            -> Replacing PENDING {removed_symbol} ({lowest_score_pending_mint[:6]}) (Score: {lowest_score:.2f}) with {symbol_for_log} ({pending_mint[:6]}) (Score: {new_candidate_score:.2f}).")
                                ...
                                else: print(f"            Skipping: No free slots and not better than existing pending.")
                            else: # No free slots, no pending entries
                                 print(f"            Skipping: Max positions ({MAX_OPEN_POSITIONS}) reached and no pending entries to replace.")
                                 
                            # Check if we are now full after add/replace, if so, break candidate loop
                            if len(state['open_positions']) + len(state.get('pending_entry', {})) >= MAX_OPEN_POSITIONS:
                                print(f"        Max positions ({MAX_OPEN_POSITIONS}) reached. Stopping candidate processing for this cycle.")
                                break 
                                
                        if added_to_pending_count > 0 or replaced_in_pending_count > 0: print(f"    [*] Finished candidate scan. Added: {added_to_pending_count}, Replaced: {replaced_in_pending_count}. Current Pending: {len(state.get('pending_entry',{}))}")
                        else: print(f"    [*] Finished candidate scan. No new pending entries added or replaced.")
                    else: print(f"    [*] No candidates passed strict filters in the scanned reports.")

            # --- Periodic Performance Summaries ---
            if current_time >= state.get('last_4hr_summary_ts', 0) + SUMMARY_INTERVAL_4HR_SECONDS:
                print(f"\n[*] Generating 4-Hour Performance Summary...")
                calculate_performance_metrics(state); generate_and_send_performance_summary_md_original(state, 4, "4-Hour", current_time) 
            elif current_time >= state.get('last_daily_summary_ts', 0) + SUMMARY_INTERVAL_DAILY_SECONDS:
                print(f"\n[*] Generating Daily Performance Summary...")
                calculate_performance_metrics(state); generate_and_send_performance_summary_md_original(state, 24, "Daily", current_time) 
            else: 
                # Save state periodically even if no summary was sent (important!)
                # print("    [*] Saving state...") # Can be noisy
                save_trader_state(state) 
                
            print(f"--- Loop End: {default_format_timestamp(time.time())} ---") # Log loop end
            time.sleep(CHECK_INTERVAL_SECONDS)
            
        except KeyboardInterrupt: print("\n--- Paper Trader stopping ---"); save_trader_state(state); print("    Trader state saved."); break
        except Exception as e:
            print(f"\n[!!!] Unhandled error in Paper Trader main loop: {e}"); traceback.print_exc()
            print("    Attempting to save state..."); save_trader_state(state)
            error_type_safe = escape_markdown_original(type(e).__name__); error_msg_safe = escape_markdown_original(str(e))
            telegram_error_message = f"🚨 *Paper Trader Main Loop Error* 🚨\n\n`{error_type_safe}: {error_msg_safe}`\n\nSee console/logs. Will retry."
            send_trader_telegram(telegram_error_message); print(f"    Retrying in {CHECK_INTERVAL_SECONDS*2} seconds..."); time.sleep(CHECK_INTERVAL_SECONDS * 2)


if __name__ == "__main__":
    main_trading_loop()

# --- END OF FILE paper_trader.py ---