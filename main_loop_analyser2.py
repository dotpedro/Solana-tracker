# --- START OF FILE main_loop_analyzer2.py ---
# Auto-generated on 2025-10-29T00:17:46.296145Z
# This is a gTFA-ready clone of your original main_loop_analyzer.py.
# The main loop logic is unchanged, but we add a small configuration block to
# advertise/use gTFA-powered helpers implemented in helius_analyzer.py.
#
# Notes:
# - The real switch to gTFA happens inside helius_analyzer.py (find_early_receivers_detailed,
#   _fetch_parsed_history_page_v0, etc.). This file simply exposes knobs and logging.
# - If helius_analyzer.py detects USE_GTFA=True, it will call getTransactionsForAddress
#   with proper order ('asc' for earliest-first scans, 'desc' for recent history).
# - Safe to run even if helius_analyzer.py is not updated; it will gracefully fall back.
...
# --- gTFA Integration Controls (read by helius_analyzer) ---------------------
# You can also override via environment variables:
#   USE_GTFA=1 | 0
#   GTFA_PAGE_LIMIT_FULL=100           (full transactions per call, max 100)
#   GTFA_PAGE_LIMIT_SIGS=1000          (signatures per call, max 1000)
#   GTFA_ORDER_EARLY='asc'|'desc'      (default 'asc' for earliest-first)
#   GTFA_ORDER_RECENT='desc'|'asc'     (default 'desc' for newest-first)

USE_GTFA = True if os.getenv("USE_GTFA", "1") == "1" else False
GTFA_PAGE_LIMIT_FULL = int(os.getenv("GTFA_PAGE_LIMIT_FULL", "100"))
GTFA_PAGE_LIMIT_SIGS = int(os.getenv("GTFA_PAGE_LIMIT_SIGS", "1000"))
GTFA_ORDER_EARLY = os.getenv("GTFA_ORDER_EARLY", "asc")
GTFA_ORDER_RECENT = os.getenv("GTFA_ORDER_RECENT", "desc")

print(f"[gTFA] USE_GTFA={USE_GTFA} FULL={GTFA_PAGE_LIMIT_FULL} SIGS={GTFA_PAGE_LIMIT_SIGS} "
